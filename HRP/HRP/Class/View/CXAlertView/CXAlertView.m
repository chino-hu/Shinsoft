...
static CGFloat const kDefaultScrollViewPadding = 2.;//与下面按钮的间距
static CGFloat const kDefaultButtonHeight = 35.;//按钮的高度
static CGFloat const kDefaultContainerWidth = 280.;//弹出框的宽度
static CGFloat const kDefaultVericalPadding = 10.;
static CGFloat const kDefaultTopScrollViewMaxHeight = 50.;//标题的最大高度
static CGFloat const kDefaultTopScrollViewMinHeight = 30.;//标题的最小高度
static CGFloat const kDefaultContentScrollViewMaxHeight = 180.;//中间主内容的最大高度
static CGFloat const kDefaultContentScrollViewMinHeight = 0.;
static CGFloat const kDefaultBottomScrollViewHeight = 35.;//放置按钮的底部的高度
...
//        _contentScrollViewMaxHeight = kDefaultContentScrollViewMaxHeight;
        _contentScrollViewMaxHeight = contentView.frame.size.height;
...
- (void)keyboardWasShow:(NSNotification *)notification{
    CGFloat height = [self preferredHeight];
    CGFloat left = (self.bounds.size.width - self.containerWidth) * 0.5;
    CGFloat top = (self.bounds.size.height - height) * 0.2;
    [UIView animateWithDuration:0.3 animations:^{
        _containerView.frame = CGRectMake(left, top, self.containerWidth, height);
        _blurView.frame = CGRectMake(left, top, self.containerWidth, height);
    }];
...
//        _titleLabel.textColor = self.titleColor;
        _titleLabel.textColor = [UIColor colorWithRed:0.0f green:0.5f blue:1.0f alpha:1.0f];
...
#endif
        _titleLabel.frame = CGRectMake( self.vericalPadding, 0, self.containerWidth - self.vericalPadding*2, [self heightWithText:self.title font:_titleLabel.font]+10);
